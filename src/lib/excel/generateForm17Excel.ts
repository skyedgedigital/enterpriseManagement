...
export async function generateForm17Excel(data: Form17Data): Promise<void> {
  const workbook = new Workbook();
  const sheet = workbook.addWorksheet("FormXVII");

  const TOTAL_COLS = 19;

  /* ── Row 1: Title ── */
  sheet.mergeCells(1, 1, 1, TOTAL_COLS);
  sheet.getCell(1, 1).value = "FORM XVII - REGISTER OF WAGES";
  ...
  // Individual columns 1-7 merge rows 4-5 (group header + sub-header)
  ...
  // Column 13 spans physical cols 13-15
  ...
  for (let logical = 14; logical <= 17; logical++) {
    const physCol = logical + 2; // 14->16, 15->17, 16->18, 17->19
