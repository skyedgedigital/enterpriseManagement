...
export function DepartmentFormPage() {
  ...
  const { selectedItem, loading } = useAppSelector((state) => state.departments);
  ...
  const onSubmit = async (data: DepartmentFormValues) => {
    if (isEditing && id) {
      const result = await dispatch(updateDepartment({ id, data }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={isEditing ? `Editing: ${selectedItem?.name}` : "Create a new department"}
        action={
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input id="name" {...register("name")} placeholder="Enter department name" />
                {errors.name && (
